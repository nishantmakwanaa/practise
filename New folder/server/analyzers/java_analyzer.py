...
def analyze_java(code):
    ...
    println_statements = re.findall(r'System\.out\.println\(', code)
    if println_statements:
        issues.append({
            "line": find_line_number(code, 'System.out.println('),
            "message": "Use a logging framework instead of System.out.println in production code",
            "severity": "info",
            "type": "readability"
        })
    ...
    for access, method_name, params in methods:
        ...
        test_code = generate_java_test(method_name, param_list)
        test_cases.append({
            "name": f"Test {method_name}()",
            "code": test_code,
            "description": f"JUnit test for the {method_name} method"
        })
...
def generate_java_test(method_name, params):
    return f"""
...
public class {method_name.capitalize()}Test {{
    ...
    public void test{method_name.capitalize()}() {{
        ...
        // Act
        Object result = instance.{method_name}({", ".join(["null" for _ in params])});
